...
class DecimalType extends avro.types.LogicalType {
    ...
    protected _fromValue(val: Buffer): any {
        return new Decimal(val.readIntBE(0, val.length), this.precision, this.scale)
    }
...
export async function fetchDataset(ccddd, dataset) {
  ...
  await new Promise((resolve, reject) => {
    avro.createBlobDecoder(blob, {
      parseHook: (schema) => avro.Type.forSchema(
        schema, { logicalTypes: { decimal: DecimalToNumberType } })
    })
      .on('metadata', (v) => { metadata = v })
