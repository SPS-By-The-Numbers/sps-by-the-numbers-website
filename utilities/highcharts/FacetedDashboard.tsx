...
export function makeOneDatasetFacetedDashboard(
  datasetSettings,
  componentGenerator,
...
export function makeMultipleDatasetFacetedDashboard(
  datasetSettingsList,
  componentGenerator,
): OptionalHighchartsComponentConfig {
...
export function makeDatasetFacetedDashboard<T extends BaseSettings>(
  ...
  if (datasetSettingsList.length == 1) {
    return makeOneDatasetFacetedDashboard(
      datasetSettingsList[0],
      componentGenerator,
    );
  } else {
    return makeMultipleDatasetFacetedDashboard(
      datasetSettingsList,
      componentGenerator,
    );
  }
