...
export default function PanoramaSlicer() {
  ...
  const handleChange = (event) => {
    ...
    if (type === "report") {
      ...
      newSelectedSubjects[parseInt(target.dataset.ordinal)] =
        event.target.value;
    ...
    if (reports === null) {
      ...
      selectedSubjects.forEach((school) => {
        ...
        Object.keys(school_data).forEach((question) => {
          ...
          // Calculate percents.
          const total_respondents = responses.answer_respondents.reduce(
            (a, n) => a + n,
            0,
          );
          ...
          data.series.push({
            name: school,
            data: responses.answer_respondents.map(
              (v) => Math.round((v * 1000) / total_respondents) / 10,
            ),
            tooltip: {
      ...
      for (let [question, data] of Object.entries(all_question_data)) {
        sorted_questions.push({
          question,
          data,
          distinctive_question_score: this.calculateDistictivenessScore(
            data.series,
          ),
        });
      }
      sorted_questions.sort(
        (a, b) => b.distinctive_question_score - a.distinctive_question_score,
      );
      ...
      sorted_questions.forEach((q) => {
        ...
        if (stacked) {
          ...
          const sortOrder = Object.keys(sortOrderMap).sort(
            (a, b) => sortOrderMap[a] < sortOrderMap[b],
          );
          ...
          graphs.push(
            <div className="h-90 flex overflow-hidden">
              <Histogram
                key={q.question}
                data={q.data}
                title={`${q.question} (diff = ${Math.round(q.distinctive_question_score * 10) / 10})`}
              />
            </div>,
          );
        }
    ...
    <main className="app-main">
      <header className="p-2 h-full w-full min-h-screen items-stretch justify-items-stretch bg-gray-300 space-x-1">
        <h4 className="p-2 font-bold">
          Seattle Public Schools Panorama Comparison Tool, 2022-2023 data (
          <a href="https://sps-panorama.web.app/">2019 here</a>)
        </h4>
        <section className="p-2 whitespace-normal tracking-normal space-x-1">
          <p>
            Data taken scraped using{" "}
            <a href="https://github.com/awong-dev/sps-by-the-numbers/blob/main/tools/scrape-panorama.js">
              a javascript blob
            </a>{" "}
            run on Panorama data viewer portal linked from the
            <a href="https://www.seattleschools.org/departments/rea/district-surveys/">
              SPS District Survey
            </a>{" "}
            page. Note in 2022, not every survey had a lot of responses. Hover
            over bar graphs to check the &quot;n&quot;.
          </p>
          <p>
            Graphs are in pecentages. Hover over data series for population
            size. When multiple series are selected, graphs are sorted to show
            questions with *most different* responses first.{" "}
            <a
              target="_blank"
              href="https://github.com/awong-dev/sps-by-the-numbers"
            >
              [source]
            </a>{" "}
            <a
              target="_blank"
              href="https://github.com/awong-dev/sps-by-the-numbers/issues"
            >
              [submit bug/feedback]
            </a>
          </p>
