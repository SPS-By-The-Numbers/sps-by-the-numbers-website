...
function calculateStats(survreyData, schools, filters) {
  const activeSchools = new Set(
    schools.filter(({ active }) => active).map((s) => s.schoolId),
  );
  ...
  for (let f of filters) {
    if (f.active === false) {
      if (f.itemId === "bus-eligible") {
        ...
        filteredData = filteredData.filter(
          (d) => d.childcareChallenges !== true,
        );
      } else if (f.itemId === "no-childcare-challenges") {
        filteredData = filteredData.filter(
          (d) => d.childcareChallenges !== false,
        );
      }
  ...
  const activeData = filteredData.filter((d) => {
    ...
    if (splitSchools.filter((v) => activeSchools.has(v)).length !== 0) {
      return true;
    }
  ...
  const numPreferCurrentBell = activeData.filter(
    (d) => d.preferCurrentBell,
  ).length;
  const numSplitBellTime = activeData.filter((d) => d.splitBellTime).length;
  const numChildCareChallenges = activeData.filter(
    (d) => d.childcareChallenges,
  ).length;
  const currentService = activeData.reduce(
    (acc, d) => {
  ...
  const distance = activeData.reduce(
    (acc, d) => {
  ...
  freeForm = freeForm.filter(
    (f) =>
      f.freeFormNoBusImpact || f.freeForm3TierImpact || f.freeFormOtherComments,
  );
  freeForm = freeForm.map((f) => ({
    ...f,
    hash: h(
      f.freeFormNoBusImpact,
      f.freeFormOtherComments,
      f.freeForm3TierImpact,
    ),
  }));
...
export default function BellTimes({
  ...
  const setFilterOption = (e) => {
    setFilters((oldFilters) =>
      oldFilters.map((f) =>
        f.itemId === e.target.name ? { ...f, value: e.target.value } : { ...f },
      ),
    );
  };
  ...
  return (
    <main>
      <section>
        <p>
          Results from the April 2022{" "}
          <a href="https://forms.gle/6siw5FAnQCqpToC46">Bell-time Survey</a>.
        </p>

        <p>
          <strong>Key result:</strong> Even families still without bus service
          overwhelmingly prefer a 2-bell schedule.
        </p>

        <p>
          <strong style={{ color: "red" }}>HAZARD:</strong> This survey method
          was very biased! It does NOT evenly represent many schools, especially
          title-1, ELL, etc. It would be wrong to interpret it as representing
          the whole district. However, it does represent over{" "}
          {survreyData.length} real families so it would be equally wrong to
          dismiss it. Read the{" "}
          <a href="https://docs.google.com/document/d/1rrpHXLxn2ajhg9V3L5rnhnA0S7K-fLPEJvNfudgVpHg/edit?#">
            executive summary
          </a>{" "}
          that was sent to the board for suggested interpretation.
        </p>
        <p>
          Other Context: Analysis from{" "}
          <a href="https://onedrive.live.com/?authkey=%21AFarbtJ1Y%2DHzy5g&cid=3381BB53AB7DC835&id=3381BB53AB7DC835%213827&parId=3381BB53AB7DC835%213778&o=OneUp&fbclid=IwAR3zazXgIDGVlWel3xXxxjqibEClcvt6gNNBIf7ZTihIm%2DX4apoXIQYDWvM">
            D2 Director Lisa Rivera Smith
          </a>
          , and{" "}
          <a href="https://docs.google.com/document/d/1KFhRiRM702CDIUMuwg5YT5ztYvmkLFcSKEw30Kpj_S0/edit?fbclid=IwAR15WA1sa2mn32aNQtZpMElG0aIVbsz9EwWfOXmORorLn7lk4qD7pnLf1j0">
            D4 Director Vivian Song Maritz
          </a>
          . Also vid meetings with{" "}
          <a href="https://www.youtube.com/watch?v=UwPz2djmtZU">
            D2,D3,D4 Lisa Rivera Smith, Chandra Hampson, and Vivian Song Maritz)
          </a>{" "}
          and{" "}
          <a href="https://www.youtube.com/watch?v=4BkX9iiLF5Y">
            D1 diredtor Liza Rankin
          </a>
          .
        </p>
          ...
          <div className="flex-stretch">
            <ItemList
              ...
              onOption={(itemId, newValue) => {
                setFilters((oldFilters) =>
                  oldFilters.map((f) =>
                    f.itemId === itemId ? { ...f, value: newValue } : { ...f },
                  ),
                );
              }}
            />
                ...
                series: [
                  {
                    showInLegend: false,
                    name: "families",
                    data: numSeries(stats.numPreferCurrentBell),
                  },
                ],
              }}
            />
                ...
                series: [
                  {
                    showInLegend: false,
                    name: "families",
                    data: numSeries(stats.numChildCareChallenges),
                  },
                ],
              }}
            />
                ...
                series: [
                  {
                    showInLegend: false,
                    name: "families",
                    data: numSeries(stats.numSplitBellTime),
                  },
                ],
              }}
            />
                ...
                series: [
                  {
                    showInLegend: false,
                    name: "families",
                    data: numSeries(stats.numEligible),
                  },
                ],
              }}
            />
          ...
          <div className="h-80 flex overflow-hidden">
            <Histogram
              ...
              data={{
                ylabel: "families",
                categories: [
                  "Eligible and will bus",
                  "Eligible and will NOT bus",
                ],
                series: [
                  {
                    showInLegend: false,
                    ...
                    data: [
                      stats.numUseIn2022,
                      stats.numEligible - stats.numUseIn2022,
                    ],
                  },
                ],
              }}
                ...
                series: [
                  {
                    ...
                    data: [
                      stats.numNeedHelp,
                      stats.numEligible - stats.numNeedHelp,
                    ],
                  },
                ],
              }}
          ...
          {stats.freeForm.map(
            ({
              ...
              <div key={freeformFingerprint} className="freeform-card">
                <h3>
                  How would not having a bus service impact your family and
                  student/s?
                </h3>
                {freeFormNoBusImpact || "[no answer]"}

                <h3>
                  How would the proposed change to bell times impact your family
                  and student/s?
                </h3>
