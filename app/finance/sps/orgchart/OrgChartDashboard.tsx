...
function getOrgChartNodes(
  ...
  const curStaffInfo: StaffInfo = {
    fte: 0,
    estSalaryBudget: 0,
    estSalaryActuals: 0,
  };
  for (const [k, v] of Object.entries(cur)) {
    const childStaffInfo = getOrgChartNodes(
      v,
      level + 1,
      s275SummaryDf,
      allNodes,
...
export default function OrgChartDashboard({
  ...
  allSettings,
  setAllSettings,
}: DistrictDataContentProps<MetricSettings>) {
  ...
  getOrgChartNodes(
    ...
    districtData
      .s275Summary()
      .filter(aq.escape((d) => d.class_of == selectedClassOf)),
    nodes,
  );
  ...
  return (
    ...
    >
      <Typography className="analysis-title" component="h1" variant="h1">
        Attempt to reverse engineer an SPS Org chart from P-A-O codes and S275
        data.
      </Typography>
