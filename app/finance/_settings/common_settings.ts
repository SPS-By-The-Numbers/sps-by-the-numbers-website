...
export function makeDefaultRevenueSettings() {
  return {
    ...
    [
      "currencyNormalization",
      {
        ...
        serialize: (settings, key) =>
          Normalizations.serializeCurrencyNormalization(settings[key]),
        deserialize: (settings, s) =>
          Normalizations.deserializeCurrencyNormalization(s),
      },
    ],
    [
      "staffingNormalization",
      {
        ...
        urlVar: "sn",
        serialize: (settings, key) =>
          Normalizations.serializeStaffingNormalization(settings[key]),
        deserialize: (settings, s) =>
          Normalizations.deserializeStaffingNormalization(s),
      },
    ],
    [
      "name",
...
export function makeRevenueSerializeConfig(context?): SettingsConfig {
  return [
    [
...
export function makeTestAdministrationSerializeConfig(
  context?,
): SettingsConfig {
  return [
    [
...
export function makeDefaultAssessmentFilterSettings() {
  return {
    gradeLevelCodes: new Set(
      ALL_GRADE_LEVELS.filter(
        (g) => g.grade_level_code === 5 || g.grade_level_code === 8,
      ).map((g) => g.grade_level_code),
    ),
    testAdministrationCodes: new Set(
      ALL_ASSESSMENT_TYPES.filter((t) => t.test_administration === "SBAC").map(
        (t) => t.test_administration_code,
      ),
    ),
    studentGroupCodes: new Set(
      ALL_STUDENT_GROUPS.filter((g) => g.student_group === "All Students").map(
...
export function serializeByConfig(
  ...
  for (const [key, serializerInfo] of serializationConfig) {
    if (serializerInfo.serializerType === "filter") {
      settingsDict[serializerInfo.urlVar] =
        serializerInfo.filter.toFilterString(settings[key]);
    } else if (serializerInfo.serializerType === "custom") {
      settingsDict[serializerInfo.urlVar] = serializerInfo.serialize(
        settings,
        key,
      );
    }
...
export function deserializeByConfig<SettingsType>(
  ...
  for (const [settingsVar, serializerInfo] of serializationConfig) {
    ...
    if (serializerInfo.serializerType === "filter") {
      settings[settingsVar] =
        serializerInfo.filter.fromFilterString(serializedValue);
    } else if (serializerInfo.serializerType === "custom") {
      settings[settingsVar] = serializerInfo.deserialize(
        settings,
        serializedValue,
      );
    }
